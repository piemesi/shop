    ...
    <div class="page-content container-fluid">
        <div class="row">
                ...
                <div class="col-md-4 col-sm-6 col-xs-12">
                    <div class="panel panel-bordered marginBottomNone" style="padding-bottom:5px;">
                        ...
                        <div class="panel-body" style="padding-top:0;">
                            ...
                            @if($row->field === 'cart')
                                @php
                                    $cartItemData = unserialize($dataTypeContent->{$row->field});

                                    foreach ($cartItemData as $key=> $productItems){

                                        if($key === 'items'){
                                            foreach ($productItems as $productItem){

                                                   //   dd($productItem['item']->id);
                                              echo '['.$productItem['item']->id.'] | '
                                           .$productItem['item']->code.' <br> '
                                            .$productItem['item']->title.' <hr>'
                                  .$allBrands[$productItem['item']->brand_id] ?? '';


                                            }
                                        }else if($key === 'totalQty') {
                                            echo ' ('. $productItems.'шт.)  <br> ';
                                        }else if($key === 'totalPrice') {
                                            echo ' '. $productItems.'  RUR ';
                                        }
                            ...
                            @elseif($row->field === 'user_id')
                                ...
                                @php
                                $user = \App\User::find($dataTypeContent->{$row->field});
